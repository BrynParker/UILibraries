...
function PANEL:Init()
	...
	self:SetHeaderHeight( self:GetTall() * 1 )
...
function PANEL:FixColumnsLayout()
	...
	for k, Column in pairs( self.Columns ) do
		...
		Column:SetTall( self:GetHeaderHeight() + 20 )
...
function PANEL:DataLayout()
	...
	for k, Line in ipairs( self.Sorted ) do
		...
		Line:SetPos( 5, y )
		Line:SetSize( self:GetWide() - 10, h )
...
function PANEL:PopulateJobs()
	self:Clear()
	for k, v in pairs( RPExtraTeams ) do
		local line = self:AddLine( v.name, v.command, v.max, v.salary, v.vote, v.admin, v.customCheck and "Yes" or "No" )
		line.job = v
	end
end
...
function PANEL:PopulatePlayers()
	...
	for k, v in pairs( player.GetAll() ) do
		local line = self:AddLine( v:Nick(), v:SteamID(), v:SteamID64(), v:Ping(), v:Frags(), v:Deaths() )
		line.player = v
	end
end

function PANEL:PopulateTable( tbl )
	self:Clear()
...
function PANEL:SetAccentColor(color)
	for k, v in pairs( self.Lines ) do
		v:AccentInternal(color)
	end
end

-- function PANEL:Paint(w,h)

-- 	TLib.DrawOutlinedBox(nil, 0, 0,w, h)
-- 	draw.RoundedBox(TLib.Config.cornerradius, 0, 0, w, self:GetHeaderHeight(), TLib.Theme.outline)
